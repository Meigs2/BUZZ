...
namespace EVEStandard
{
    ...
    public class DesktopSSO
    {
        ...
        /// </remarks>
        internal DesktopSSO(string callbackUri, string clientId, DataSource dataSource)
        {
            ...
            RandomBase64String = CreateRandomBase64String();
        }
        ...
        internal string RandomBase64String { get; }
        ...
        public Authorization AuthorizeToEVEUri(List<string> scopes, string state)
        {
            ...
            // Generate our code challenge
            SHA256 hasher = new SHA256CryptoServiceProvider();
            var hashResult = hasher.ComputeHash(Encoding.UTF8.GetBytes(RandomBase64String));
            var codeChallenge = Convert.ToBase64String(hashResult);

            model.SignInURI = GetBaseURL() + SSO_AUTHORIZE + "response_type=code&redirect_uri=" +
                              HttpUtility.UrlEncode(CallbackUri) +
                              "&client_id=" + ClientId + "&scope=" + HttpUtility.UrlEncode(String.Join(" ", scopes)) +
                              "&code_challenge=" + HttpUtility.UrlEncode(codeChallenge) + "&code_challenge_method=S256" +
                              "&state=" + HttpUtility.UrlEncode(model.ExpectedState);

            return model;
        }

        private string CreateRandomBase64String()
        {
            char[] chars =
        ...
        public async Task<AccessTokenDetails> VerifyAuthorizationAsync(Authorization model)
        {
            ...
            try
            {
                var stringContent = new FormUrlEncodedContent(new[]
                {
                    ...
                    new KeyValuePair<string, string>("code_verifier", RandomBase64String)
                });
